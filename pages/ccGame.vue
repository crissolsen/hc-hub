    ...
    <div id="game-wrapper" v-if="ready">
      <div v-for="index in divLength" :key="index">
        <div
          class="colour-block"
          :id="`block-${index}`"
          :style="`background: ${initialRandomiser()}`"
          @click="setRandomColour(index)"
        >
          <h4>{{ actions[~~(Math.random() * actions.length)] }}</h4>
        </div>
...
export default {
  ...
  data() {
    return {
      sound: new Audio("http://www.synthcom.com/Europa/sounds/RandomSound7.mp3"),
      divLength: 3,
      ready: false,
      actions: [
        "jump",
  ...
  methods: {
    setRandomColour(index) {
      ...
      selectedBlock.innerHTML =
        "<h4>" +
        this.actions[~~(Math.random() * this.actions.length)] +
        "</h4>";
    },
    initialRandomiser() {
      return (
    ...
    lengthCheck() {
      if (this.divLength < this.divLengthMax) {
        ...
        alert(
          `The maximum number of steps is ${this.divLengthMax}, please try again.`
        );
      }
    },
  },
